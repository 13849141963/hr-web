...
public class DownloadFileUtils {
    ...
    public static void downloadIDCardPdfZip(HttpServletRequest request, HttpServletResponse response, List<PdfFileParamDtoReq> listFileParam) {

        List<List> list = groupListByQuantity(listFileParam, 40);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            try {
                //添加ZipEntry，并ZipEntry中写入文件流
                //这里，加上i是防止要下载的文件有重名的导致下载失败
                zipos.putNextEntry(new ZipEntry("身份证集" +i+ ".pdf"));
                os = new DataOutputStream(zipos);
                // InputStream is = ImgUtilOpenPlateform.downloadFile(loadHouseholdResp.get(i).getSelfHuKouPicId());
                //InputStream is = ApiUtilOpenPlateform.callOpenPlateformApiGetInoutStream("publicmgr.mergePdfFileByFileId",JSON.toJSONString(listPdf));
                InputStream is = getPdfInputStream(list.get(i));
                byte[] b = new byte[100];
                int length = 0;
                while ((length = is.read(b)) != -1) {
                    os.write(b, 0, length);
    ...
    //批量下载HSSFWorkbook
    public static void downloadHSSFWorkbook(HttpServletRequest request, HttpServletResponse response, List<HSSFWorkbook> list) {
        ...
        //设置压缩包的名字
        //解决不同浏览器压缩包名字含有中文时乱码的问题
        String downloadName = "工伤新参表集.zip";
        String agent = request.getHeader("USER-AGENT");
        try {
            if (agent.contains("MSIE") || agent.contains("Trident")) {
                downloadName = java.net.URLEncoder.encode(downloadName, "UTF-8");
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            try {
                //添加ZipEntry，并ZipEntry中写入文件流
                //这里，加上i是防止要下载的文件有重名的导致下载失败
                zipos.putNextEntry(new ZipEntry( list.get(i).getSheetName(0)+ ".xls"));
                os = new DataOutputStream(zipos);
                HSSFWorkbook wb = list.get(i);
                wb.write(os);
                zipos.closeEntry();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static List<List> groupListByQuantity(List list, int quantity) {
        ...
        List<List> wrapList = new ArrayList<List>();
        int count = 0;
